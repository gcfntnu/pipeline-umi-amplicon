...
def parse_args(argv):
    ...
    parser.add_argument("-m", "--min", dest="MIN",
                        help="Min read count for clusters")
    parser.add_argument("-M", "--max", dest="MAX",
                        help="Max read count for clusters")
    #parser.add_argument("-n", "--name", dest="NAME",
    #                    help="Amplicon name. Name used in the target BED file.)",)
    parser.add_argument("-n", "--name", dest="NAME",
                        help="Amplicon names. BED file.)",)
    parser.add_argument("-f", "--figures", dest="FIGURES",
                        help="Prefix for output figures")
    parser.add_argument("FOLDERS", type=str, nargs="+",
                        help="UMI pipeline output folder")
...
def get_readstast_bam(folder):
    bam_1d_file = "{}/align/1_d.bam".format(folder)
...
def count_fastx_reads_with_umi(folder, amplicon_name):
    fastq_with_umi_file = "{}/fasta_umi/{}_detected_umis.fasta".format(folder, amplicon_name)
...
def get_median_acc(folder, amplicon_name):
    acc_stats_file = "{}/stats/{}_consensus_size_vs_acc.tsv".format(folder, amplicon_name)
...
def cluster_read_hist(folder, amplicon_name, display_name, figure_prefix):
    ...
    ax.axvline(2, ls="--", zorder=1, label="Min cluster size", color="red", alpha=0.8)
    #ax.axvline(10, ls="--", zorder=1, label="Max cluster size", color="red", alpha=0.8)
    #ax.set_xlim(0, 18)
...
def main(argv=sys.argv[1:]):
    ...
    #name = args.NAME
    NAMES = read_bed_names(args.NAME)
    c_min = int(args.MIN)
    c_max = int(args.MAX)
    ...
    for folder in folders:
        ...
        for i, name in enumerate(NAMES):
            ...
            try:
                ...
                print('progress: {}%'.format(i/float(len(NAMES))))
            except:
                print('failed stats for: {}'.format(name))
        fname = os.path.join(folder, 'summary_stats.tsv')
        print(df_stats.head())
        print("writing to file {} ... ".format(fname))
        df_stats.to_csv(fname, sep="\t", index=False, header=True)
